...
Les graphiques présentant les principaux résultats sont tous accessibles en scrollant. Cliquez sur les menus déroulants pour obtenir plus d'informations.
...
# Avant d'explorer les modélisations
...
**L'intérêt des modélisations n'est-il pas d'empêcher qu'elles se produisent ?** *(cliquer pour dérouler)*
...
Bien évidemment, les projections décrivant ce qu'il devrait se passer en l'absence de mesures ne peuvent être comparées avec ce qui s'est réellement passé dans les pays confinés. C'est d'ailleurs un contre-argument commun : c'est précisément grâce aux mesures que les scénarios de modélisation pessimistes ne se sont pas réalisés.
...
**Comparaison à la réalité** *(cliquer pour dérouler)*
...
f_graph(
  ...
  "rea", 1, 0,
  "2020-03-27", 500, #réalité label
  "2020-06-01", 1500, #scénarios label
  "2020-04-29", 1000, #date de publication label
  "2020-03-19", "2020-06-28", NA, #limites
  "Lits de réanimation en Ile-de-France", 
  "scénarios non publiés de l'Institut Pasteur, retranscrit dans les Echos \n"
)
```
...
f_graph(
  ...
  "new_rea_center", 2, 0,
  "2020-10-18", 320, #réalité label
  "2020-12-01", 750, #scénarios label
...
**Comparaison à la réalité : INSERM** *(cliquer pour dérouler)*
...
f_graph(
  ...
  "Nombre hebdomadaire d'admission à l'hôpital", 
  "scénarios publiés par l'INSERM le 16 janvier 2021\n"
) 
...
f_graph(
  ...
  "2021-01-01", "2021-03-27", 37000, #limites
  "Nombre hebdomadaire d'admission à l'hôpital", 
  "scénarios publiés par l'INSERM le 2 février 2021\n"
) 
...
f_graph(
  ...
  "2021-01-01", "2021-03-27", 37000, #limites
  "Nombre hebdomadaire d'admission à l'hôpital", 
  "scénarios publiés par l'INSERM le 14 février 2021\n"
) 
```
...
**Comparaison à la réalité : l'Institut Pasteur** *(cliquer pour dérouler)*
...
Pour le [rapport du 23 février](https://hal-pasteur.archives-ouvertes.fr/pasteur-03149525/document), **le scénario médian correspond globalement à la réalité**, qui se trouve aux alentours du scénario médian. 

Il n'est pas fait mention de la différence importante avec les scénarios du rapport précédent du 8 février.
...
f_graph(
  ...
  "Admission journalières à l'hôpital", 
  "scénarios publiés par l'Institut Pasteur le 8 février 2021\n"
) +
  geom_line(
    data= true_data_new_hosp_rea_no_mean, 
    aes(
...
f_graph(
  ...
  "2021-01-10", "2021-03-27", 3500, #limites
  "Admission journalières à l'hôpital", 
  "scénarios publiés par l'Institut Pasteur le 23 février 2021\n"
) +
  geom_line(
    data= true_data_new_hosp_rea_no_mean, 
    aes(
...
-   **En s'appuyant sur ces résultats, ils donnaient également des recommandations politiques explicites** : "*Ces résultats montrent la nécessité de renforcer les mesures de distanciation sociale*", et ce bien que le couvre-feu, dont le but même est d'éviter des mesures plus restrictives, n'était pas modélisé.
...
En revanche la **décrue observée suit de près les scénarios du second rapport**. Il n'y est pas fait mention de la différence importante entre les scénarios des 2 rapports consécutifs.
...
f_graph(
  ...
  "Admissions journalières à l'hôpital", 
  "scénarios publiés par l'Institut Pasteur le 26 avril 2021\n"
) +
  geom_line(
    data= true_data_new_hosp_rea_no_mean, 
    aes(
...
f_graph(
  ...
  "2021-04-01", "2021-06-15", 2500, #limites
  "Admissions journalières à l'hôpital", 
  "scénarios publiés par l'Institut Pasteur le 21 mai 2021\n"
) +
  geom_line(
    data= true_data_new_hosp_rea_no_mean, 
    aes(
...
Pour le rapport du [26 avril](https://modelisation-covid19.pasteur.fr/loosening/Scenarios_de_levee_des_mesures_de_freinage_20210426.pdf), les courbes sont extraites des figures 3b et 3d. Les hypothèses faites dans les scénarios sont les suivantes :

-   Un variant alpha plus transmissible de 60% ou 40% comparé à la souche originelle. Il augmente aussi de 67% la probabilité d'hospitalisation.
...
-    Efficacité vaccinale de 90% contre les cas sévères et 80% contre les infections, 2 semaines après la dose 1. Les personnes infectées ont une immunité totale.

-    Adhésion vaccinale de 85% pour les > 65 ans, et 70% pour les 18-64 ans.
...
Les hypothèses faites dans le rapport du [21 mai](https://modelisation-covid19.pasteur.fr/loosening/Mise_a_jour_scenarios_de_levee_des_mesures_de_freinage_20210521.pdf) sont sensiblement les mêmes. Les courbes viennent des figures 3A, 3C, 3E, 3G.

Certaines hypothèses sont décrites comme potentiellement trop optimistes par les modélisateurs, comme par exemple celle de l'adhésion vaccinale. Il est aussi précisé que le modèle ne prend pas en compte la fin des restrictions de voyage, ou la réouverture des écoles.

Dans les 2 cas nous arrêtons la comparaison à la mi-juin, [lorsque l'apparition du variant delta](https://ourworldindata.org/explorers/coronavirus-data-explorer?zoomToSelection=true&time=2021-04-26..2021-08-16&facet=none&pickerSort=asc&pickerMetric=location&Metric=Delta+variant+%28share%29&Interval=7-day+rolling+average&Relative+to+Population=true&Color+by+test+positivity=false&country=~FRA) non prévue par les modèles et devenu dominant rend les hypothèses caduques.
...
**Comparaison à la réalité (cliquer pour dérouler)** *(cliquer pour dérouler)*

</summary>

Pour le premier rapport, la réalité est presque **2 fois moins importante que les scénarios les plus optimistes**, **3 à 4 fois** moins importante que les **scénarios médians**, **10 fois moins** importante que les **scénarios pessimistes**.

Le **rapport publié 10 jour plus tard corrige partiellement** cette surestimation, la réalité correspondant environ au scénario le plus optimiste, mais restant 2 fois moins importante que le scénario médian et 4 fois moindre que le scénario pessimiste.
...
**Impact politique et médiatique (cliquer pour dérouler) ** *(cliquer pour dérouler)*

</summary>

En ligne avec les projections présentées, le président du conseil scientifique [Jean-François Delfraissy prévoyait](https://www.publicsenat.fr/article/parlementaire/covid-19-jean-francois-delfraissy-prevoit-une-situation-tres-complexe-dans-les) alors une situation hospitalière compliquée pour fin août lors de **son audition devant les sénateurs** :
...
Cette mise en avant des projections de l'Institut Pasteur au Sénat est intervenue avant que celui-ci ne se prononce sur l'extension du pass sanitaire, et a donc pu jouer un rôle dans l'avis des sénateurs.

Les modélisations de l'Institut Pasteur ont été également [utilisées par le Conseil d'Etat comme argument](https://www.conseil-etat.fr/content/download/160458/file/454792-454818.pdf) pour **rejeter les demandes de référés-libertés au sujet de l'extension du pass sanitaire**. Pour rejeter ces demandes, le Conseil d'Etat a notamment expliqué que
...
Le scénarios du [premier rapport](https://modelisation-covid19.pasteur.fr/variant/Institut_Pasteur_Acceleration_vaccination_et_Delta_20210726.pdf) sont extraits des figures 3, 4, 5 et 6. Les scénarios dont les hypothèses ne collaient pas à la réalité ont été exclus de la comparaison, permettant de ramener le nombre de variantes de 108 à seulement 6 pour les admissions et 12 pour le nombre de lits occupés. Dans le détail, ces hypothèses sont :

-   Durée de passage en soins critiques : 14.6 jours ou 10 jours

-   Réduction du nombre de reproduction effectif le portant à 1.5, 1.8 ou 2 (0, -10% ou -20%)en raison des mesures barrières et du pass sanitaire.
...
-   Doses de vaccins distribués par jour : 500 000, 700 000 ou 800 000. Nous avons éliminé les scénarios à 800 000 doses/jour, trop éloignés de la réalité (voir graphique ci-dessous). Celui à 700 000 ne correspond qu'au début du mois d'août, mais nous avons fait le choix de le conserver, afin de ne pas risquer d'exlure de scénarios optimistes plus proches de la réalité.

Les courbes du [second rapport](https://modelisation-covid19.pasteur.fr/variant/InstitutPasteur_Dynamiques_regionales_des_hospitalisations_20210805.pdf) sont issues de la figure F. Le rapport fait uniquement des hypothèses sur 

-   durée de passage en soins critiques (10, 14 ou 17 jours)

-   nombre de doses de vaccin distribuées par jour (600 000, 700 000 800 000). Au vu du second graphique ci-dessous, nous avons exclu les scénarios à 700 000 et 800 000 doses/jour.

-   réduction du nombre de reproduction effectif (-10%, -25% ou -40%) en raison des mesures barrières et du pass sanitaire.

-   Adhésion vaccinale : 95% pour les +60 ans, 90% pour les 18-60 ans, 30% pour les 12-17 ans. A noter que ces chiffres sont similaires à ceux que nous avons retenus pour le scénario du 26 avril.
...
Par ailleurs un certain nombre d'hypothèses étaient communes aux 2 scénarios : 

-   **Le variant delta n'est pas considéré comme plus susceptible d'entraîner une hospitalisation que le variant alpha (+64% pour les 2 variants).**

-   Efficacité vaccinale 2 semaines après la dose 1 : 90% contre hospitalisation, 80% contre infection, 50% contre transmission. 

-   Le modèle ne suppose pas une baisse d'efficacité vaccinale face au variant delta. Il est dit que **"cela pourrait conduire à présenter des projections trop optimistes pour le mois d’août"**.

-   Les personnes infectées sont considérées comme totalement immunisées. Il est dit que "cela pourra conduire à faire des projections trop optimistes".
...
f_graph(
  ...
  "Admissions à l'hôpital",
  "scénarios publiés par l'Institut Pasteur le 4 octobre 2021\n"
) +
  geom_line(
    data= true_data_new_hosp_rea_no_mean, 
    aes(
...
Nos courbes sont issues des figures 7 et 9.

-   Des comportements/mesures réduisant les transmissions au niveau de juin/juillet avant le pass sanitaire (-40% par rapport à juin/juillet), ou à un niveau intermédiaire (-20%)
...
-   Efficacité vaccinale contre la transmission de 50% si l'on est infecté.

-   Le variant delta augmente le risque d'hospitalisation de 50% comparé au variant alpha.

-   Le variant alpha augmente le risque d'hospitalisation de 42% comparé à la souche historique.


Comme nous l'avons déjà dit, le "relâchement des gestes barrières" est une hypothèse impossible à vérifier. Une mesure imparfaite de cette hypothèse serait la tendance à la mobilité de Google, qui ne montre pas d'augmentation nette au cours de la période :
...
Le rapport distinguait des scénarios "probables" de scénarios "moins probables". Il distinguait également une hypothèses d'efficacité à 6 mois de la dose de rappel contre les infections de 85% ou 60%.
...
Le pic de lits de soins critiques interviendra quelques jours après la publication du rapport au lieu du mois et demi anticipé. La **hauteur du pic sera légèrement en-dessous du scénario le plus optimiste**, mais plus de **2 fois moindre que les scénarios médians et pessimistes**. Les scénarios "peu probables" surestiment encore plus la réalité.
...
f_graph_unsmoothed(
  ...
  "Lits de soins critiques", 
  "scénarios publiés par l'Institut Pasteur le 7 janvier 2022\n"
)
...
f_graph_unsmoothed(
  ...
  annotate(
    geom="text", x=as.Date("2022-03-15"), y=9200, label="HYPOTHESE\nEFFICACITE\nVACCINALE\nFAUSSE !",
...
**Auto-évaluation de l'Institut Pasteur et hypothèses** *(cliquer pour dérouler)*
